...
class PlayBigRaffleAlways extends Command
{
     ...
     * @var string
     */
    protected $signature = 'raffle:big:always-win {--players=10000}';
...
//            $this->ask('Begin?');
            for ($raffleNo = 1; $raffleNo <= $numberOfTickets; ++$raffleNo) {
...
//                $this->line("The drawn ticket is $drawnTicket (of $raffleCount)");
        ...
        for ($a = 1; $a <= 5; ++$a) {
            ...
            if ($pid === 0) {
                ...
                for ($b = 1; $b <= 5; ++$b) {
                    $drawingsTilWon = $play();
                    $actualities[$b] = $drawingsTilWon;

                    $average = array_sum($actualities) / count($actualities);
                    $game = $a * $b;
                    $this->line("Game $game won on drawing #$drawingsTilWon (average: $average).");
//                    sleep(1);
                }

                Redis::set("actualities-$a", json_encode($actualities));
                exit;
            }
        }
        ...
        print_r($actualities);
        $average = array_sum($actualities) / count($actualities);
        $this->line("Average plays-to-win: $average");
    }
